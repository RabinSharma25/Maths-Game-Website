...
function generateQA() {
    ...
    var val = Math.floor(randomNumber(1, 5));
    if (val == 1) { // for multipliction 
    ...
    else if (val == 4) { // for division
        var x = 1 + Math.round(9 * Math.random());
        var y = 1 + Math.round(9 * Math.random());
        correctAnswer =Math.floor(x/y);
        ...
        for (i = 1; i < 5; i++) {
            if (i != correctPosition) {
                var wrongAnswer;
                do {
                    wrongAnswer = Math.floor((1 + Math.round(9 * Math.random())) / (1 + Math.round(9 * Math.random()))); //a wrong answer
                } while (answers.indexOf(wrongAnswer) > -1)
                document.getElementById("box" + i).innerHTML = wrongAnswer;
                answers.push(wrongAnswer);
            }
